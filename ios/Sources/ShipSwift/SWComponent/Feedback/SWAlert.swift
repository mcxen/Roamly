...
final class SWAlertManager: ObservableObject {
    ...
    @Published private(set) var backgroundStyle = SWAlertType.info.backgroundStyle
...
private struct SWAlertView: View {
    ...
    var body: some View {
        if alertManager.isShowing {
            ...
            .padding(.vertical, 10)
            .background(alertManager.backgroundStyle, in: Capsule())
            .overlay(
                Capsule()
                    .stroke(alertManager.borderColor, lineWidth: 0.5)
            )
